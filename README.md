...
네이버 키가 **지도/경로용**과 **검색용**으로 따로 있으면 아래처럼 넣으면 됩니다. 특히 지도 화면 인증 에러가 나면 `NAVER_MAP_NCP_KEY_ID`에 Web Dynamic Map의 `ncpKeyId` 값을 명시해 주세요.

| 넣을 이름 | 네이버에서 가져올 값 | 어디에 쓰이나요? | 필수 여부 |
| --- | --- | --- | --- |
| `NAVER_MAP_NCP_KEY_ID` | Naver Cloud Platform Maps Application의 **Web Dynamic Map ncpKeyId** | 브라우저 네이버 지도 SDK 인증 | 지도 인증이 계속 실패하면 명시 |
| `NAVER_MAP_CLIENT_ID` | Naver Cloud Platform Maps Application의 **Client ID / API Key ID** | Geocoding, Directions 5 REST API 인증 ID | 필수 |
...
- **지도 관련 키**는 `NAVER_MAP_CLIENT_ID`, `NAVER_MAP_CLIENT_SECRET`에 넣고, 지도 화면 인증이 실패하면 `NAVER_MAP_NCP_KEY_ID`도 추가합니다.
...
```bash
NAVER_MAP_NCP_KEY_ID=네이버_Web_Dynamic_Map_ncpKeyId
NAVER_MAP_CLIENT_ID=네이버_지도_REST_API_Client_ID
...
```bash
Name: NAVER_MAP_NCP_KEY_ID
Value: 네이버 Cloud Maps Web Dynamic Map의 ncpKeyId

Name: NAVER_MAP_CLIENT_ID
Value: 네이버 Cloud Maps의 REST API Client ID / API Key ID
...
```bash
NAVER_MAP_NCP_KEY_ID=여기에_네이버_Web_Dynamic_Map_ncpKeyId
NAVER_MAP_CLIENT_ID=여기에_네이버_지도_REST_API_Client_ID
...
위 주소는 키 값 자체는 보여주지 않고, `NAVER_MAP_NCP_KEY_ID`, `NAVER_MAP_CLIENT_ID`, `NAVER_MAP_CLIENT_SECRET`, `NAVER_SEARCH_CLIENT_ID`, `NAVER_SEARCH_CLIENT_SECRET`이 들어왔는지만 `true/false`로 보여줍니다. 실제 네이버 API 호출까지 확인하려면 아래처럼 `live=1`을 붙입니다.
...
1. **잘못된 지도 SDK 키**
   ...
   - Vercel에 `NAVER_MAP_NCP_KEY_ID`를 추가하고, 값은 Naver Cloud Platform Maps Application의 Web Dynamic Map `ncpKeyId`를 넣으세요.
   - REST API용 Client ID와 같은 값이면 `NAVER_MAP_CLIENT_ID`만 있어도 되지만, 헷갈리면 `NAVER_MAP_NCP_KEY_ID`를 따로 넣는 쪽이 안전합니다.
...
```txt
index.html
  ├─ /api/naver-maps.js  → NAVER_MAP_NCP_KEY_ID 또는 NAVER_MAP_CLIENT_ID로 네이버 지도 JavaScript SDK 로드
